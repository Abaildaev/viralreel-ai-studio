...
`funnel-media` отличается от остальных ещё двумя вещами: политики ограничивают доступ собственной папкой пользователя на всех глаголах, как и для `templates`/`audio`, и у него выставлен `file_size_limit` в 20 МБ, потому что лимиты редактора живут в браузере и защитой не являются.
...
Связи между tenant-строками проверяются не только политикой браузера: триггеры
на `scheduled_posts`, шаблонах, лид-магнитах и Telegram-сущностях отклоняют
чужой `instagram_account_id`/`telegram_bot_id` даже для service-role воркера.
...
**Рассылки** (`telegram-broadcast`) материализуют аудиторию в очередь при старте и шлют ~25 сообщений в секунду. Перед отправкой воркер атомарно арендует строки через `SKIP LOCKED`; зависшие аренды возвращаются в очередь через пять минут. Это исключает параллельную отправку одним и тем же получателям при перекрывающихся cron-тиках (после сбоя Telegram всё равно остаётся моделью at-least-once). Claim-функции и сами воркеры дополнительно сверяют владельца бота, подписчика, воронки и шага до расшифровки токена. Для drip-воронки claim разрешает только самый ранний активный шаг: промпт не может опередить инструкцию даже после backfill. Возвращённый Telegram `message_id` сохраняется на строке доставки для расследования сбоев. Сегмент аудитории описан данными в `_shared/broadcast-segments.ts`, и композер с воркером читают одно и то же описание — иначе показанное число получателей и реальная выборка разошлись бы.

**Аналитика автоматизаций** считается функцией `get_automation_analytics` внутри Postgres. Клиент получает дневные агрегаты, KPI и топы, а не весь журнал событий. CRM использует `get_automation_leads` с серверной группировкой контактов, поиском, фильтром и страницами по 50 строк. Обе функции привязывают выборку к `auth.uid()` через владельца Instagram-аккаунта.
