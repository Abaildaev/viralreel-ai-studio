...
export const AutomationRuleEditorModal: React.FC<AutomationRuleEditorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
        {/* Modal Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="p-2 bg-slate-50 border border-slate-200 rounded-xl flex items-center flex-wrap gap-2 focus-within:border-[#1E60FF]">
              {form.keywords.map((kw) => (
                <span
                  key={kw}
                  className="bg-blue-50 text-[#1E60FF] border border-blue-100 px-2 py-0.5 rounded-md text-xs font-mono font-semibold flex items-center gap-1.5"
                >
                  {kw}
                  <button
                    type="button"
          ...
          <div className="space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <input
                ...
                className="bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs outline-none focus:border-[#1E60FF]"
              />
              <input
                type="url"
            ...
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-slate-700 flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="rounded text-[#1E60FF] focus:ring-0"
                />
              ...
              {form.public_reply_enabled && (
                <button
                  ...
                  className="text-xs text-[#1E60FF] font-semibold hover:underline flex items-center gap-1"
                >
                ...
                {form.public_reply_variants.map((v, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <input
                      ...
                      className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-xs outline-none focus:border-[#1E60FF]"
                    />
          ...
          <div className="grid sm:grid-cols-2 gap-4 pt-3 border-t border-slate-100">
            <div>
              ...
              />
              <p className="text-[10px] text-slate-400 mt-0.5">Имитация живого ответа (0 = сразу)</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-xs outline-none"
              />
              <p className="text-[10px] text-slate-400 mt-0.5">Не слать повторно одному человеку</p>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button
              ...
              className="px-5 py-2 bg-[#1E60FF] hover:bg-[#1551E5] disabled:bg-slate-200 text-white font-semibold rounded-xl text-xs shadow-xs transition-all flex items-center gap-2"
            >
